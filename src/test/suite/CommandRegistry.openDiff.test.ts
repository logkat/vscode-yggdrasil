...
suite('CommandRegistry — ygg.openDiff', () => {
  test('calls vscode.diff with correct URIs and preview: true', async () => {
    ...
    const [leftUri, rightUri, title, opts] = diffCall!.args as [vscode.Uri, vscode.Uri, string, unknown];
  ...
  test('does nothing when called with no arguments', async () => {
    ...
    assert.strictEqual(calls.filter(c => c === 'vscode.diff').length, 0, 'vscode.diff should not have been called');
  });

  test('diff title format: "<branch> — <relativePath> (branch base ↔ working tree)"', async () => {
    const reg = new CommandRegistry(makeMockContext(), makeMockGit(), makeMockProvider());
    const item = makeFileItem('components/Button.tsx', 'A', '/home/user/projects/feature-x', 'feature/add-button');

    let capturedTitle = '';
    const origExecute = vscode.commands.executeCommand;
    (vscode.commands as any).executeCommand = async (cmd: string, _l: unknown, _r: unknown, title: string) => {
