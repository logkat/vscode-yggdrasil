...
function makeProvider(files: Record<string, object>): ClaudeCodeProvider {
  ...
  const readFile = async (p: string) => {
    ...
    if (!(key in files)) { throw Object.assign(new Error(), { code: 'ENOENT' }); }
...
suite('ClaudeCodeProvider', () => {
  test('returns session when cwd matches', async () => {
    const provider = makeProvider({
      '1234.json': { sessionId: 'abc-123', cwd: '/repo/wt/feat-x', name: 'Fix auth', status: 'idle', updatedAt: 1000 },
    });
  ...
  test('returns empty array when sessions dir is missing', async () => {
    const readdir = async (): Promise<string[]> => { throw Object.assign(new Error(), { code: 'ENOENT' }); };
